    ...
    <div class="container mx-auto px-4 py-24">
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2">
                ...
                <div class="bg-white rounded-lg shadow-md p-8 mb-8">
                    <div class="flex items-center justify-between mb-6">
                        <h3 class="text-2xl font-bold text-gray-900">Komentar ({{ $article->comments->count() }})</h3>
                        <button class="text-teal-600 hover:text-teal-700 font-semibold" onclick="document.getElementById('comment-form').scrollIntoView();">
                             Tambahkan Komentar di bawah ini !!!
                        </button>
                    </div>

                    <!-- Comment Form -->
                    @auth
                    <form id="comment-form" action="{{ route('articles.comment', $article->id) }}" method="POST" class="mb-6">
                        @csrf
                        <div class="flex gap-4">
                            <!-- Input -->
                            <div class="flex-1">
                                <textarea name="comment_content" rows="3" placeholder="Tulis komentar..." 
                                        class="w-full border border-gray-300 rounded-lg p-3 focus:ring-2 focus:ring-teal-500 focus:border-teal-500 resize-none"></textarea>
                                <button type="submit" class="mt-2 bg-teal-600 text-white px-4 py-2 rounded-lg hover:bg-teal-700 transition">
                                    Kirim
                                </button>
                            </div>
                        </div>
                    ...
                    @else
                        <p class="text-sm text-gray-500 mb-6">Silakan <a href="{{ route('login') }}" class="text-teal-600 underline">login</a> untuk menulis komentar.</p>
                    @endauth
            ...
            <div class="lg:col-span-1">
                <div class="bg-teal-700 text-white rounded-lg shadow-lg p-6 sticky top-4 space-y-8">

                    <!-- Fun Fact -->
                    <div>
                        <h3 class="text-xl font-bold mb-4 border-b border-teal-500 pb-2">Fun Fact</h3>
                        <div class="space-y-4 text-sm">
                            <div>
                                <h4 class="font-semibold">🕺 Tari Saman</h4>
                                <p>Tari Saman dilakukan tanpa alat musik, hanya suara dan gerakan tubuh.</p>
                            </div>
                            <div>
                                <h4 class="font-semibold">🌏 Bahasa Aceh</h4>
                                <p>Bahasa Aceh memiliki lebih dari 7 dialek berbeda tergantung daerahnya.</p>
                            </div>
                            <div>
                                <h4 class="font-semibold">🎉 Festival Budaya</h4>
                                <p>Beberapa festival diadakan tiap tahun untuk melestarikan tarian dan musik tradisional.</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <!-- Artikel Terkait -->
                    <div>
                        <h3 class="text-xl font-bold mb-4 border-b border-teal-500 pb-2">Artikel Terkait</h3>
                        <div class="space-y-4">
                            @foreach($relatedArticles as $rel)
                                <a href="{{ route('pustakawarisan.show', $rel->id) }}" 
                                class="flex items-start gap-3 bg-white text-gray-900 rounded-lg shadow hover:bg-gray-100 p-3 transition">
                                
                                    <!-- Thumbnail -->
                                    <div class="w-16 h-16 flex-shrink-0 overflow-hidden rounded-lg">
                                        <img src="{{ $rel->img_url ?? asset('images/default-thumbnail.png') }}" 
                                            alt="{{ $rel->title }}" 
                                    ...
                                    <!-- Judul & Deskripsi -->
                                    <div class="flex-1">
                                        <h4 class="font-semibold text-sm mb-1">{{ Str::limit($rel->title, 50) }}</h4>
                                        <p class="text-xs text-gray-600">{{ Str::limit(strip_tags($rel->content), 60) }}</p>
                                    </div>
                                </a>
                            @endforeach
                        </div>
                    ...
                    <!-- Bagikan Artikel -->
                    <div>
                        <h3 class="text-xl font-bold mb-4 border-b border-teal-500 pb-2">Bagikan Artikel</h3>
                        <div class="flex flex-col sm:flex-row sm:gap-4 gap-2">
                            ...
                            target="_blank" 
                            class="flex items-center justify-center gap-2 bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded-lg transition">
                            ...
                            target="_blank" 
                            class="flex items-center justify-center gap-2 bg-pink-500 hover:bg-pink-600 text-white px-4 py-2 rounded-lg transition">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
